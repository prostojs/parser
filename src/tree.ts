...
const styles = {
    ...
    valuesDim: (s: string) =>
        __DYE_CYAN__ + __DYE_DIM__ + s + __DYE_COLOR_OFF__ + __DYE_DIM_OFF__,
    ...
    values: (s: string | number) =>
        (typeof s === 'string' ? __DYE_CYAN_BRIGHT__ : __DYE_YELLOW_BRIGHT__) +
        cut(s.toString(), 30) +
        __DYE_COLOR_OFF__,
    nodeDim: (s: string) =>
        __DYE_YELLOW__ + __DYE_DIM__ + s + __DYE_COLOR_OFF__ + __DYE_DIM_OFF__,
    node: (s: string) => __DYE_YELLOW__ + s + __DYE_COLOR_OFF__,
}

const stringOutputLimit = 70

export const parserTree: ProstoTree<ProstoParserNodeContext | string | 0> =
    new ProstoTree<ProstoParserNodeContext | string | 0>({
        children: 'content',
        renderLabel: (context) => {
            if (typeof context === 'string') {
                return styles.text(
                    '«' + cut(context, stringOutputLimit) + __DYE_GREEN__ + '»',
                )
            } else if (
                typeof context === 'object' &&
                context instanceof ProstoParserNodeContext
            ) {
                let keys = ''
                ...
                Object.keys(data).forEach((key) => {
                    ...
                    if (typeof val === 'string' || typeof val === 'number') {
                        ...
                        keys +=
                            ' ' +
                            `${styles.underscore(st(key))}${st(val ? '☑' : '☐')}`
                    }
                })
                return (
                    styles.node(context.icon + (context.label ? ' ' : '')) +
                    styles.nodeDim(context.label) +
                    keys
                )
            }
            return ''
