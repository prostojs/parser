...
type MapRule<CustData extends TGenericCustomDataType> =
    | ''
    | 'content.join'
    | 'customData'
    | ((ctx: ProstoParserNodeContext<CustData>) => unknown)
type MapRuleTyped<
    CustData extends TGenericCustomDataType,
    ChildData,
> = keyof ChildData extends string
    ? MapRule<CustData> | `customData.${keyof ChildData}`
    : MapRule<CustData>

export interface TProstoParserHoistOptions<
    CustData extends TGenericCustomDataType = TDefaultCustomDataType,
    ChildData extends TGenericCustomDataType = TDefaultCustomDataType,
> {
...
export type TAbsorbRules<ChildData> = keyof ChildData extends string
    ?
          | 'append'
          | 'join'
          | `copy->${keyof ChildData}`
          | `join->${keyof ChildData}`
    : 'append' | 'join'
...
export type TMapContentRules =
    ...
    | 'pop'
    | 'copy'
export type TMapContentRule<T extends TGenericCustomDataType> =
    | ((content: ProstoParserNodeContext<T>['content']) => unknown)
    | TMapContentRules

export type TMapContentOptions<
    T extends TGenericCustomDataType = TDefaultCustomDataType,
> = {
    [key in keyof T]: TMapContentRule<T>
}

export interface TProstoParserNodeOptions<
    T extends TGenericCustomDataType = TDefaultCustomDataType,
> {
    label?: string
    icon?: string
    ...
    onBeforeChildParse?: (
        childContext: ProstoParserNodeContext,
        data: TPorstoParserCallbackData<T>,
    ) => void
    onAfterChildParse?: (
        childContext: ProstoParserNodeContext,
        data: TPorstoParserCallbackData<T>,
...
> {
    token:
        ...
        | string[]
        | RegExp
        | ((context: ProstoParserNodeContext<T>) => string | string[] | RegExp)
    ignoreBackSlashed?: boolean
    omit?: boolean
    eject?: boolean
    consume?: boolean
    onMatchToken?: (
        data: TPorstoParserCallbackDataMatched<T>,
    ) => boolean | { omit?: boolean; eject?: boolean; consume?: boolean } | void
}

export interface TPorstoParserCallbackDataMatched<
    T extends TGenericCustomDataType = TDefaultCustomDataType,
> extends TPorstoParserCallbackData<T> {
