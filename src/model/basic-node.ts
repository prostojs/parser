...
export type TOmitEjectShortcut =
    | ''
    | `${TOmit | TEject | TConsume | ''}-${TOmit | TEject | TConsume | ''}`
export type TBackSlashShortcut =
    | ''
    | `${TBackSlashIgnore | ''}-${TBackSlashIgnore | ''}`

export interface TBasicNodeOptions<
    T extends TGenericCustomDataType = TDefaultCustomDataType,
> {
    label?: string
    icon?: string
    tokens?: [
        TProstoParserTokenDescripor<T>['token'],
        TProstoParserTokenDescripor<T>['token'],
    ]
...
export class BasicNode<
    T extends TGenericCustomDataType = TDefaultCustomDataType,
> extends ProstoParserNode<T> {
    constructor(options: TBasicNodeOptions<T>) {
        const startsWith: TProstoParserTokenDescripor | undefined =
            options?.tokens ? { token: options?.tokens[0] } : undefined
        const endsWith: TProstoParserTokenDescripor | undefined =
            options?.tokens ? { token: options?.tokens[1] } : undefined
