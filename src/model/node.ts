...
> extends ProstoParserNodeBase<T> {
    ...
    public getOptions(): Required<TProstoParserNodeOptions<T>> {
        return {
            ...
            startsWith: (this.options.startsWith
                ...
                : this.options.startsWith) as Required<
                TProstoParserNodeOptions<T>
            >['startsWith'],
            endsWith: (this.options.endsWith
                ? { ...this.options.endsWith }
                : this.options.endsWith) as Required<
                TProstoParserNodeOptions<T>
            >['endsWith'],
            ...
            onPop: this.options.onPop as Required<
                TProstoParserNodeOptions<T>
            >['onPop'],
            onMatch: this.options.onMatch as Required<
                TProstoParserNodeOptions<T>
            >['onMatch'],
            onAppendContent: this.options.onAppendContent as Required<
                TProstoParserNodeOptions<T>
            >['onAppendContent'],
            onAfterChildParse: this.options.onAfterChildParse as Required<
                TProstoParserNodeOptions<T>
            >['onAfterChildParse'],
            onBeforeChildParse: this.options.onBeforeChildParse as Required<
                TProstoParserNodeOptions<T>
            >['onBeforeChildParse'],
            initCustomData: this.options.initCustomData as Required<
                TProstoParserNodeOptions<T>
            >['initCustomData'],
            absorbs: this.options.absorbs as Required<
                TProstoParserNodeOptions<T>
            >['absorbs'],
        }
    }
    ...
    public createContext(
        index: number,
        level: number,
        rootContext?: ProstoParserContext,
    ): ProstoParserNodeContext<T> {
    ...
    public get name() {
        return (
            ...
            ']' +
            '(' +
            (this.options.label || this.options.icon || '') +
            ')'
        )
    }
    ...
    public beforeOnAppendContent(
        s: string,
        _data: TPorstoParserCallbackData<T>,
    ): string {
        // to be overriden
        return s
    }

    public beforeOnAfterChildParse(
        _child: ProstoParserNodeContext,
        _data: TPorstoParserCallbackData<T>,
    ) {
        // to be overriden
    }

    public beforeOnBeforeChildParse(
        _child: ProstoParserNodeContext,
        _data: TPorstoParserCallbackData<T>,
    ) {
        // to be overriden
