...
export function renderCodeFragment(
    lines: string[],
    options: TCodeFragmentOptions,
) {
    ...
    if (delta > limit) {
        ...
        for (let i = 0; i < newLimit + offset; i++) {
            ...
            output += renderLine(
                ...
                index === row ? error : undefined,
                index === row || index === rowEnd ? 'bold' : '',
            )
        }
        let output2 = ''
        for (let i = newLimit + offset; i > 0; i--) {
            ...
            output2 += renderLine(
                line,
                index < maxIndex ? index + 1 : '',
                index === rowEnd ? errorEnd : undefined,
                index === row || index === rowEnd ? 'bold' : '',
            )
        }
        output +=
            renderLine('—'.repeat(longestLine), '———', undefined, 'dim') +
            output2
    } else {
...
function lineStyles(s: string) {
    return (
        __DYE_BLUE_BRIGHT__ +
        (s || '')
            .replace(/([a-z_]+)/gi, __DYE_GREEN__ + '$1' + '\x1b[39;33m')
            .replace(/([=./'"`:+]+)/gi, __DYE_CYAN__ + '$1' + __DYE_COLOR_OFF__)
    )
}
...
function renderLine(
    ...
    const st =
        (style === 'bold' ? __DYE_BOLD__ : '') +
        (style === 'dim' ? __DYE_DIM__ : '')
    if (typeof error === 'number') {
        const errorLength =
            (
                /[.-\s()*/+{}[\]?'"`<>]/.exec(line.slice(error + 1)) || {
                    index: line.length - error,
                }
            ).index + 1
        return (
            renderLineNumber(index, true) +
            st +
    ...
    return (
        renderLineNumber(index, undefined, style === 'bold') +
        st +
        lineStyles(line)
    )
}
